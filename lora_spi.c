// Taken from: https://github.com/daveake/pico-tracker/blob/main/lora.c
#include <stdio.h>
#include <string.h>
...
int BuildLoRaPositionPacket(struct SENSINFO *BMEINFO, unsigned char *TxLine)
{
    /*
...
int TimeToSend(struct SENSINFO *BMEINFO)
{
    int CycleSeconds;
...
char* debugLoraMode(tLoRaMode LoRaMode)
{
    switch (LoRaMode)
    {
        case lmSending: return "Sending";
        case lmIdle: return "Idle";
        case lmListening: return "Listening";
    }
}

int LoRaIsFree(struct SENSINFO *BMEINFO)
{
    uint checkPIN = gpio_get(PIN_DIO0);
    char *damode = debugLoraMode(LoRaMode);
    printf("checking lora is free or not... LoRaMode: %s\n\n", damode);

    bool enterMainLoop = false;

    if (LoRaMode != lmSending){
        enterMainLoop = true;
        printf("LoRa is not sending, entering main loop...\n");
    }else{
        if (checkPIN){
            enterMainLoop = true;
            printf("DIO0 GPIO is HIGH, entering loop..., val: %d\n",checkPIN);
        }else{
            printf("DIO0 GPIO is LOW, not entering..., val: %d\n",checkPIN);
        }
    }

    if (enterMainLoop)
    {
        printf("lora is not sending...\n");
        ...
        printf("lora check time...\n");
        if (TimeToSend(BMEINFO))
        {
            printf("lora yes time...\n");
...
void SendLoRaPacket(unsigned char *buffer, int Length, int CallingPacket)
{
    ...
    */
    ...
    int checkWritten;
    checkWritten = spi_write_blocking(SPI_PORT, buf, 1);
    printf("written bytes: %i", checkWritten);
    checkWritten = spi_write_blocking(SPI_PORT, buffer, Length);
    printf("written bytes: %i", checkWritten);

    cs_deselect();

    // go into transmit mode
    printf("go to transmit mode...\n");
...
void check_lora(struct SENSINFO *BMEINFO)
{
    // CheckFSKBuffer();

    // CheckLoRaRx();

    if (LoRaIsFree(BMEINFO))
    {
        printf("LoRa is free\n");
        if (SendRepeatedPacket == 3)
        {
            printf("SendRepeatedPacket: 3\n");
        ...
        {
            printf("Sending no repeated packet (0)\n");
            int PacketLength;
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        // 0x80 | (LORA_ID << 3) | TargetID
                        PacketLength = BuildLoRaPositionPacket(BMEINFO, Sentence);
                        printf("LoRa: Tx Binary packet");
                    }
                    else
                    {
                        PacketLength = BuildSentence(BMEINFO, (char *)Sentence, PayloadID);
                        // printf("LoRa: Tx ASCII Sentence\n");
                        printf("\n%s\r\n", (char *)Sentence);
                    }
                    printf("==================================================\n");
                    printf("Called >>> SendLoRaPacket <<< \n");
